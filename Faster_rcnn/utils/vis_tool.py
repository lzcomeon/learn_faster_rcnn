...
from datasets.voc_dataset import VOC_BBOX_LABEL_NAMES
...
def vis_bbox(img, bbox, label=None, score=None, ax=None):
    ...
    if label is not None and not len(bbox) == len(label):
        raise ValueError('The length of label must be the same as that of bbox')
    ...
    for i, bb in enumerate(bbox):
        ...
        ax.add_patch(plot.Rectangle(xy,
